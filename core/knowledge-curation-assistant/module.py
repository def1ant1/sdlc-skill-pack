...
SCHEMA = {
    "module": "knowledge_curation_assistant",
...
def process_action(state: dict[str, Any], payload: dict[str, Any], interaction: dict[str, Any], ctx: P0OperationContext) -> dict[str, Any]:
    enforce_policy("knowledge_curation_assistant", payload)
    issues=qa_check(payload, interaction)
    if issues:
        raise ValueError("; ".join(issues))
    new_state,_ = apply_reversible_update(state, payload)
    ctx.log("knowledge_curation_assistant", "process_action", {"payload": payload, "interaction": interaction})
    return new_state
