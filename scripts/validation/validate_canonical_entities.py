...
EXAMPLES = ROOT / "examples" / "canonical-entities"


def main() -> int:
    if not EXAMPLES.exists():
        print("No canonical entity examples found; skipping.")
        return 0
    ...
    for sample in sorted(EXAMPLES.glob("*.json")):
        ...
        if "lineage" not in doc:
            failures.append(f"{sample}: $.lineage missing required field")
    ...
    if failures:
        print("\n".join(failures))
        return 1
    print("Canonical entity validation passed.")
    return 0
