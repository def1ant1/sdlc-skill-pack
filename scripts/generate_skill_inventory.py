...
def main():
    ...
    for typ in ['core','skills','agents']:
        ...
        for child in sorted(d.iterdir(), key=lambda p:p.name):
            ...
            if child.is_dir() and sk.exists():
                m=fm(sk); rows.append({'category':typ,'id':child.name,'name':m.get('name',child.name),'version':m.get('version'),'owner':m.get('owner'),'maturity':m.get('maturity')})
    ...
    with (a.root/'reports'/'skill_inventory.csv').open('w',newline='',encoding='utf-8') as f:
        w=csv.DictWriter(f,fieldnames=['category','id','name','version','owner','maturity']); w.writeheader(); w.writerows(rows)
    md=['# Skill Inventory','',f'Total: {len(rows)}','', '| category | id | name | version | owner | maturity |','|---|---|---|---|---|---|']
    md += [f"| {r['category']} | {r['id']} | {r['name']} | {r['version'] or ''} | {r['owner'] or ''} | {r['maturity'] or ''} |" for r in rows]
